...
from utils.inference_utils import upsample_color_image, merge_channels_into_color_image
from utils.util import robust_min, robust_max
from utils.timers import Timer, CudaTimer, cuda_timers
...
class ImageReconstructor:
    def __init__(self, model, height, width, num_bins, options):
        ...
        # torch.device('cuda:0') if self.use_gpu else
    ...
    def initialize(self, height, width, options):
        ...
        self.perform_color_reconstruction = options.color
        if self.perform_color_reconstruction:
            if options.auto_hdr:
                print('!!Warning: disabling auto HDR for color reconstruction!!')
            # disable auto_hdr for color reconstruction (otherwise, each channel will be normalized independently)
            options.auto_hdr = False

        self.crop = CropParameters(
            self.width, self.height, self.model.num_encoders)
        ...
        self.unsharp_mask_filter = UnsharpMaskFilter(
